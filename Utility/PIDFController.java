...
public class PIDFController {
    ...
    public PIDFController(PIDFCoefficients pid){
        m_P = pid.p;
        m_I = pid.i;
        m_D = pid.d;
        m_F = pid.f;
    ...
    private void calculate()
    {
        ...
        if (m_enabled)
        {
            ...
            if ((Math.abs(m_totalError + m_error) * m_I < m_maximumIntegral) &&
                    (Math.abs(m_totalError + m_error) * m_I > m_minimumIntegral))
                m_totalError += m_error;
            // m_totalError = Range.clip(m_totalError, 0.0, .4);

            // Perform the primary PID calculation
            m_result = m_P * m_error + m_I * m_totalError + m_D * (m_error - m_prevError) + m_F * getSetpoint();
     ...
     * This is always centered on zero and constrained to the max and min outs
     * @return the latest calculated output
     */
    ...
    public void setIntegralRange(double _min, double _max){
        m_minimumIntegral = _min;
        m_maximumIntegral = _max;
    }
    /**
    ...
    public boolean onTarget(int cnt){
        if((Math.abs(m_error) < Math.abs(m_tolerance / 100.0 * (m_maximumInput - m_minimumInput)))){
            m_onTargetCnt++;
        }
        if(m_onTargetCnt >= cnt) {
            m_onTargetCnt = 0;
            return true;
        }
        else
